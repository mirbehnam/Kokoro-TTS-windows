...
def generate_tts_with_logs(voice_name, text, format, speed):
    ...
    try:
        if model is None:
            ...
            model = build_model("kokoro-v0_19.pth", device)  # Make sure the model path is correct
...
# Create the interface with added YouTube-like channel section
    ...
    Subscribe
        ...
        with gr.Row():
            with gr.Column(scale=1):
                with gr.Group():
                    voice = gr.Dropdown(
                        choices=["af_bella", "af_khanyi", "en_us_male"],
        ...
        generate_button.click(
            fn=lambda voice, text, format, speed: ("Generating audio...", "path/to/audio.wav"),
